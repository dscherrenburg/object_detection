...
class CopyPaste(BaseMixTransform):
    ...
    def __init__(self, dataset=None, pre_transform=None, p=0.5, mode="flip") -> None:
        ...
        super().__init__(dataset=dataset, pre_transform=pre_transform, p=p)
        assert mode in {"flip", "mixup", "bbox"}, f"Support mode `flip`, `mixup` or `bbox`, got {mode}"
        self.mode = mode
    ...
    def __call__(self, labels):
        ...
        if len(labels["instances"].segments) == 0 or self.p == 0:
            return labels
        if self.mode == "flip" or self.mode == "bbox":
    ...
    ### This function needs to be updated if the CopyPaste will be used.
    ### Mode bbox is customly made and currently requires labels2 to be non-empty.
    def _transform(self, labels1, labels2={}):
        """Applies Copy-Paste augmentation. Supports 'flip', 'mixup', and 'bbox' modes."""
        im = labels1["img"]
        h, w = im.shape[:2]
        ...
        if self.mode != "bbox":
            # Original mask-based mode using segments
            ...
            # New bbox-only mode (for Faster R-CNN)
            im_new = im.copy()
            instances2 = labels2.pop("instances", None)
            if instances2 is None:
                instances2 = deepcopy(instances)
            ...
            bboxes2 = instances2.bboxes
            cls2 = labels2.get("cls", cls)

            ioa = bbox_ioa(bboxes2, bboxes)
            indexes = np.nonzero((ioa < 0.30).all(1))[0]
            n = len(indexes)
            sorted_idx = np.argsort(ioa.max(1)[indexes])
            indexes = indexes[sorted_idx]
            ...
            new_cls = []
            for j in indexes[: round(self.p * n)]:
                box = bboxes2[j].astype(int)
                x1, y1, x2, y2 = box
                if x2 <= x1 or y2 <= y1:
                    continue
                patch = labels2.get("img", im)[y1:y2, x1:x2]
                if patch.size == 0:
                    continue
                im_new[y1:y2, x1:x2] = patch
                new_boxes.append(box)
                new_cls.append(cls2[j])

            if new_boxes:
                instances = Instances.concatenate(
                    (instances, Instances(bboxes=np.array(new_boxes), format="xyxy"))
                )
                cls = np.concatenate((cls, np.array(new_cls)), axis=0)
...
class Resize:
    """
    Resize an image while maintaining its aspect ratio without padding.

    Arguments:
        size (int): The target size for the longer side of the image.

    Methods:
        __call__: Resize image, update labels and bounding boxes.
    """
    ...
    def __call__(self, labels=None):
        """
        Resize the image and update labels and bounding boxes.

        Args:
            labels (Dict | None): A dictionary containing image data and associated labels.

        Returns:
            Dict: Updated dictionary with resized image and related metadata.
        """
        if labels is None:
            labels = {}
        ...
        if img is None:
            raise ValueError("No image found in 'labels' dictionary under key 'img'.")
        ...
        if len(labels):
            labels = self._update_labels(labels, (ratio, ratio))
            labels["img"] = img
            labels["resized_shape"] = (new_h, new_w)
            labels["ratio_pad"] = ((ratio, ratio), (0, 0))
        return labels

    @staticmethod
    def _update_labels(labels, ratio):
        """
        Updates labels after applying letterboxing to an image.

        This method modifies the bounding box coordinates of instances in the labels
        to account for resizing.
        ...
        Examples:
            >>> resize = Resize(new_shape=(640, 640))
            >>> labels = {"instances": Instances(...)}
            >>> ratio = (0.5, 0.5)
            >>> updated_labels = resize._update_labels(labels, ratio)
        """
        labels["instances"].convert_bbox(format="xyxy")
        labels["instances"].denormalize(*labels["img"].shape[:2][::-1])
        labels["instances"].scale(*ratio)
...
def fasterrcnn_transforms(dataset, imgsz, hyp):
    """
    Applies a series of image transformations optimized for Faster R-CNN.

    Args:
        dataset (Dataset): The dataset object containing image data and annotations.
        imgsz (int): The target image size for resizing.
        hyp (Namespace): A dictionary of hyperparameters controlling various aspects of the transformations.
        stretch (bool): If True, applies stretching to the image. If False, uses standard resizing.

    Returns:
        (Compose): A composition of image transformations to be applied to the dataset.
    """
    # Resize transformation (preserve aspect ratio) (removed as FasterRCNN does it in first layer)
    resize = Resize(size=imgsz)

    # Random affine transformations (mild distortions)
    affine = RandomPerspective(
        degrees=hyp.degrees,  # Reduce intensity
        translate=hyp.translate,  # Reduce intensity
        scale=hyp.scale,  # Reduce intensity
        shear=hyp.shear,  # Reduce intensity
        perspective=hyp.perspective,  # Reduce intensity
        pre_transform=resize,
    )

    # Copy-Paste with bounding box adjustments (function needs to be updated if it will actually be used)
    pre_transform = Compose([affine])
    if hyp.copy_paste_mode == "bbox":
        copy_paste = CopyPaste(dataset, p=hyp.copy_paste, mode="bbox")  # Ensure bbox correctness
        pre_transform.append(copy_paste)
    ...
    # Random flipping and color augmentation
    return Compose(
        [
            pre_transform,
            MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup),
