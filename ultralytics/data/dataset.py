...
class FasterRCNNDataset(YOLODataset):
    """
    FasterRCNN dataset class extending the base YOLODataset class.

    This specialized dataset class is designed for use with the FasterRCNN object detection model.
    """
    def __init__(self, *args, data=None, **kwargs):
        """Initialize the FasterRCNNDataset class by inheriting from the YOLODataset class."""
        super().__init__(*args, data=data, **kwargs)
        self.nc = self.data["nc"]+1  # number of classes
    ...
    def build_transforms(self, hyp=None):
        ...
        """
        if self.augment:
            transforms = fasterrcnn_transforms(self, self.imgsz, hyp)
        else:
            # transforms = Compose([Resize(size=self.imgsz)])
            transforms = Compose([Resize(size=self.imgsz)])
        transforms.append(
            Format(
                bbox_format="xyxy",
    ...
    def collate_fn(batch):
        """
        Collates data samples into batches and adds 'targets' for Faster R-CNN.

        Args:
            batch (List[dict]): List of dictionaries containing sample data.

        Returns:
            dict: Collated batch with 'targets' (list of dicts with 'boxes' and 'labels').
        """
        # Extract classes and bounding boxes from each sample
        clss = [b["cls"].squeeze(1) if b["cls"].ndim > 1 else b["cls"] for b in batch]  # Convert [N, 1] -> [N]
        bboxes = [b["bboxes"] for b in batch]

        # Create Faster R-CNN targets
        targets = [
            {"boxes": b.to(torch.float32), 
             "labels": c.to(torch.int64) + 1 }  # Faster R-CNN expects labels to be 1-indexed
            for b, c in zip(bboxes, clss)
        ]

        # Add 0 padding to 'ratio_pad' as LetterBox is not used
        for b in batch:
            b["ratio_pad"] = None
            # b["img"] = b["img"][..., [2, 1, 0]]  # swap BGR -> RGB

        # Use existing collate function and add targets
        new_batch = YOLODataset.collate_fn(batch)
        new_batch["img"] = new_batch["img"] / 255.0  # Normalize images to [0, 1]
