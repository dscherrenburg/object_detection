...
class FasterRCNNDetectionLoss:
    """Class for computing the loss for Faster R-CNN model."""
    ...
    def __call__(self, losses=None, batch=None, **kwargs):
        """
        Compute the loss for the Faster R-CNN model.
        ...
        return self._loss_adapter(losses)
    
    def _loss_adapter(self, loss_dict):
        """
        Adapt the loss dictionary to the desired format.

        Args:
            loss_dict (dict): Dictionary containing computed losses.

        Returns:
            dict: Adapted loss dictionary.
        """
        ...
        loss[2] = obj_loss * self.hyp.obj
        loss[3] = rpn_loss * self.hyp.rpn

        batch_size = self.hyp.batch
        loss = loss.cpu().float()
        loss, loss_items = loss * batch_size, loss.detach()
