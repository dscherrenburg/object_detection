from .utils import BBFormat, BBType, CoordinatesType, convertToAbsoluteValues, convertToRelativeValues
...
class BoundingBox:
    ...
    def __repr__(self):
        if self._typeCoordinates == CoordinatesType.Absolute:
            x, y, w, h = round(float(self._x), 2), round(float(self._y), 2), round(float(self._w), 2), round(float(self._h), 2)
        elif self._typeCoordinates == CoordinatesType.Relative:
            x, y, w, h = round(float(self._x), 5), round(float(self._y), 5), round(float(self._w), 5), round(float(self._h), 5)
        conf = round(float(self._classConfidence), 3) if self._classConfidence is not None else None
        ...
        if self._format == BBFormat.XYWH:
            ...
            format = "XYX2Y2"
        if self._typeCoordinates == CoordinatesType.Relative:
            typeCoordinates = "Relative"
        else:
            typeCoordinates = "Absolute"
        return f"{bbType} BoundingBox(image={self._imageName}, classId={self._classId}, x={x}, y={y}, w={w}, h={h}, confidence={conf}, typeCoordinates={typeCoordinates}, imgSize=({self._width_img}, {self._height_img}), format={format})"
